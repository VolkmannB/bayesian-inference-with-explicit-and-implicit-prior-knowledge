...
imes_colorscale = matplotlib.colors.LinearSegmentedColormap.from_list(
    "imes_Colorscale", [imes_blue, imes_green, imes_orange], N=256
)
...
def plot_Data(Particles, weights, Reference, time, axes):
    ...
    if N_dim != len(axes):
        raise ValueError(
            "Number of states must be equal to the number of the given axes"
        )
    ...
    for i in range(N_dim):
        axes[i].plot(time, mean[:, i], color=imes_blue, label="mean")
        axes[i].fill_between(
            time,
            mean[:, i] - 3 * std[:, i],
            mean[:, i] + 3 * std[:, i],
        ...
        axes[i].plot(
            time, Reference[:, i], color="red", linestyle="--", label="true"
        )
...
def plot_fcn_error_1D(X_in, Mean, Std, X_stats, X_weights, ax, ax_histx):
    ...
    for i in range(0, len(ax)):
        # plot function
        ax[i].plot(X_in, Mean[i], color=imes_blue, label="mean")
        ax[i].fill_between(
            X_in,
            Mean[i] - 3 * Std[i],
            Mean[i] + 3 * Std[i],
...
def calc_wRMSE(w, y1, y2):
    ...
    wRMSE = np.sqrt(
        1 / (v1 - (v2 / v1**2)) * np.sum((y1 - y2) ** 2 * w, axis=-1)
    )
