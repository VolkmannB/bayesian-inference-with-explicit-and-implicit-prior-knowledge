...
from src.Publication_Plotting import (
    imes_blue,
    calc_wRMSE,
)
...
SMO_offline_logL = data["offline_log_likelihood"]
...
SMO_online_logL = data["online_log_likelihood"]
...
(
    SMO_offline_GP_Mean,
    SMO_offline_GP_Col_Cov,
    SMO_offline_GP_Row_Scale,
    SMO_offline_GP_df,
) = jax.vmap(prior_mniw_2naturalPara_inv)(
    SMO_GP_prior_stats[0]
    + np.cumsum(SMO_offline_T0, axis=0)
    / np.arange(1, SMO_offline_Sigma_X.shape[1] + 1)[:, None, None],
    SMO_GP_prior_stats[1]
    + np.cumsum(SMO_offline_T1, axis=0)
    / np.arange(1, SMO_offline_Sigma_X.shape[1] + 1)[:, None, None],
    SMO_GP_prior_stats[2]
    + np.cumsum(SMO_offline_T2, axis=0)
    / np.arange(1, SMO_offline_Sigma_X.shape[1] + 1)[:, None, None],
    SMO_GP_prior_stats[3]
    + np.cumsum(SMO_offline_T3, axis=0)
    / np.arange(1, SMO_offline_Sigma_X.shape[1] + 1),
)
...
(
    SMO_online_GP_Mean,
    SMO_online_GP_Col_Cov,
    SMO_online_GP_Row_Scale,
    SMO_online_GP_df,
) = jax.vmap(prior_mniw_2naturalPara_inv)(
...
Veh_offline_logL = data["offline_log_likelihood"]
...
Veh_online_logL = data["online_log_likelihood"]
...
Veh_GP_prior_stats_f = [
    data["prior_T0_f"],
    data["prior_T1_f"],
    data["prior_T2_f"],
    data["prior_T3_f"].flatten(),
]
Veh_GP_prior_stats_r = [
    data["prior_T0_r"],
    data["prior_T1_r"],
    data["prior_T2_r"],
    data["prior_T3_r"].flatten(),
]
...
# Convert sufficient statistics to standard parameters
(
    Veh_offline_GP_Mean_f,
    Veh_offline_GP_Col_Cov_f,
    Veh_offline_GP_Row_Scale_f,
    Veh_offline_GP_df_f,
) = jax.vmap(prior_mniw_2naturalPara_inv)(
    Veh_GP_prior_stats_f[0]
    + np.cumsum(Veh_offline_T0_f, axis=0)
    / np.arange(1, Veh_offline_Sigma_X.shape[1] + 1)[:, None, None],
    Veh_GP_prior_stats_f[1]
    + np.cumsum(Veh_offline_T1_f, axis=0)
    / np.arange(1, Veh_offline_Sigma_X.shape[1] + 1)[:, None, None],
    Veh_GP_prior_stats_f[2]
    + np.cumsum(Veh_offline_T2_f, axis=0)
    / np.arange(1, Veh_offline_Sigma_X.shape[1] + 1)[:, None, None],
    Veh_GP_prior_stats_f[3]
    + np.cumsum(Veh_offline_T3_f, axis=0)
    / np.arange(1, Veh_offline_Sigma_X.shape[1] + 1),
)
(
    Veh_offline_GP_Mean_r,
    Veh_offline_GP_Col_Cov_r,
    Veh_offline_GP_Row_Scale_r,
    Veh_offline_GP_df_r,
) = jax.vmap(prior_mniw_2naturalPara_inv)(
    Veh_GP_prior_stats_r[0]
    + np.cumsum(Veh_offline_T0_r, axis=0)
    / np.arange(1, Veh_offline_Sigma_X.shape[1] + 1)[:, None, None],
    Veh_GP_prior_stats_r[1]
    + np.cumsum(Veh_offline_T1_r, axis=0)
    / np.arange(1, Veh_offline_Sigma_X.shape[1] + 1)[:, None, None],
    Veh_GP_prior_stats_r[2]
    + np.cumsum(Veh_offline_T2_r, axis=0)
    / np.arange(1, Veh_offline_Sigma_X.shape[1] + 1)[:, None, None],
    Veh_GP_prior_stats_r[3]
    + np.cumsum(Veh_offline_T3_r, axis=0)
    / np.arange(1, Veh_offline_Sigma_X.shape[1] + 1),
)
...
# Convert sufficient statistics to standard parameters
(
    Veh_online_GP_Mean_f,
    Veh_online_GP_Col_Cov_f,
    Veh_online_GP_Row_Scale_f,
    Veh_online_GP_df_f,
) = jax.vmap(prior_mniw_2naturalPara_inv)(
...
(
    Veh_online_GP_Mean_r,
    Veh_online_GP_Col_Cov_r,
    Veh_online_GP_Row_Scale_r,
    Veh_online_GP_df_r,
) = jax.vmap(prior_mniw_2naturalPara_inv)(
...
EMPS_offline_logL = data["offline_log_likelihood"]
...
EMPS_online_logL = data["online_log_likelihood"]
...
EMPS_offline_Sigma_X_PGAS = data["offline_Sigma_X_PGAS"]
EMPS_offline_logL_PGAS = data["offline_log_likelihood_PGAS"]

EMPS_RMSE_Alg2 = data["RMSE_Alg2"]
EMPS_RMSE_PGAS = data["RMSE_PGAS"]
print(f"EMPS_RMSE_Alg2: {EMPS_RMSE_Alg2}\nEMPS_RMSE_PGAS: {EMPS_RMSE_PGAS}")
...
# Convert sufficient statistics to standard parameters
(
    EMPS_offline_GP_Mean,
    EMPS_offline_GP_Col_Cov,
    EMPS_offline_GP_Row_Scale,
    EMPS_offline_GP_df,
) = jax.vmap(prior_mniw_2naturalPara_inv)(
    EMPS_GP_prior_stats[0]
    + np.cumsum(EMPS_offline_T0, axis=0)
    / np.arange(1, EMPS_offline_Sigma_X.shape[1] + 1)[:, None, None],
    EMPS_GP_prior_stats[1]
    + np.cumsum(EMPS_offline_T1, axis=0)
    / np.arange(1, EMPS_offline_Sigma_X.shape[1] + 1)[:, None, None],
    EMPS_GP_prior_stats[2]
    + np.cumsum(EMPS_offline_T2, axis=0)
    / np.arange(1, EMPS_offline_Sigma_X.shape[1] + 1)[:, None, None],
    EMPS_GP_prior_stats[3]
    + np.cumsum(EMPS_offline_T3, axis=0)
    / np.arange(1, EMPS_offline_Sigma_X.shape[1] + 1),
)
...
(
    EMPS_online_GP_Mean,
    EMPS_online_GP_Col_Cov,
    EMPS_online_GP_Row_Scale,
    EMPS_online_GP_df,
) = jax.vmap(prior_mniw_2naturalPara_inv)(
...
gs_traj = fig_traj.add_gridspec(
    3, 3, width_ratios=np.ones(3), height_ratios=np.ones(3)
)
...
SMO_cax_title_ax = fig_fcn.add_subplot(gs_fcn_0[0, 2])
SMO_cax_title_ax.text(0.5, 0.5, "Error in N", ha="center", va="center")
SMO_cax_title_ax.axis("off")
...
SMO_ax_S1_histx.set_title("Nonlinear Oscillator")
Veh_ax_S1_histx.set_title("Vehicle")
EMPS_ax_S1_histx.set_title("Real-world Positioning System")
...
plot_Data(
    Particles=np.concatenate(
        [SMO_offline_Sigma_X, SMO_offline_Sigma_F], axis=-1
    ),
...
SMO_ax_x0.set_title("Nonlinear Oscillator")
...
plot_Data(
    Particles=np.concatenate(
        [Veh_offline_Sigma_X, Veh_offline_Sigma_mu_f], axis=-1
    ),
...
plot_Data(
    Particles=np.concatenate(
        [EMPS_offline_Sigma_X, EMPS_offline_Sigma_F], axis=-1
    ),
    weights=EMPS_offline_weights,
    Reference=np.concatenate(
        [EMPS_X, np.ones((EMPS_Y.shape[0], 1)) * np.nan], axis=-1
    ),
    time=EMPS_time,
...
EMPS_ax_x0.plot(
    EMPS_time,
    np.mean(EMPS_offline_Sigma_X_PGAS, axis=1)[:, 0],
    color="g",
    linestyle=":",
)
EMPS_offline_Sigma_X_baseline_var = np.var(EMPS_offline_Sigma_X_PGAS, axis=1)
EMPS_ax_x0.fill_between(
    EMPS_time,
    np.mean(EMPS_offline_Sigma_X_PGAS, axis=1)[:, 0]
    - 2 * np.sqrt(EMPS_offline_Sigma_X_baseline_var[:, 0]),
    np.mean(EMPS_offline_Sigma_X_PGAS, axis=1)[:, 0]
    + 2 * np.sqrt(EMPS_offline_Sigma_X_baseline_var[:, 0]),
    color="g",
    alpha=0.2,
...
EMPS_ax_x1.plot(
    EMPS_time,
    np.mean(EMPS_offline_Sigma_X_PGAS, axis=1)[:, 1],
    color="g",
    linestyle=":",
    label=r"PGAS\,[39] $\pm 3\sigma$",
)
EMPS_ax_x1.fill_between(
    EMPS_time,
    np.mean(EMPS_offline_Sigma_X_PGAS, axis=1)[:, 1]
    - 2 * np.sqrt(EMPS_offline_Sigma_X_baseline_var[:, 1]),
    np.mean(EMPS_offline_Sigma_X_PGAS, axis=1)[:, 1]
    + 2 * np.sqrt(EMPS_offline_Sigma_X_baseline_var[:, 1]),
    color="g",
    alpha=0.2,
...
EMPS_ax_x0.set_title("Real-world Positioning System")
EMPS_ax_x0.tick_params(axis="x", bottom=False, labelbottom=False)
EMPS_ax_x1.tick_params(axis="x", bottom=False, labelbottom=False)
EMPS_ax_x1.set_ylim(-0.31, 0.5)
handles, labels = EMPS_ax_x1.get_legend_handles_labels()
new_handles = [
    (handles[0], EMPS_ax_x1.collections[0]),
    handles[1],
    (handles[2], EMPS_ax_x1.collections[1]),
]
...
apply_basic_formatting(fig_traj, width=18, height=8.4, font_size=8)
...
# calculate wRMSE
SMO_offline_wRMSE = calc_wRMSE(
    1 / SMO_offline_fcn_var, SMO_offline_fcn_mean, SMO_F_sd_true_plot
)

# normalize variance to create transparency effect
fcn_alpha = np.maximum(
    np.minimum(1 - SMO_offline_fcn_var / SMO_fcn_var_prior, 1), 0
)
...
# calculate wRMSE
Veh_offline_wRMSE_f = calc_wRMSE(
    1 / Veh_offline_fcn_var_f, Veh_offline_fcn_mean_f, Veh_mu_true_plot
)
Veh_offline_wRMSE_r = calc_wRMSE(
    1 / Veh_offline_fcn_var_r, Veh_offline_fcn_mean_r, Veh_mu_true_plot
)
...
Veh_ax_S1_plt.set_xticks(
    [-0.15, 0, 0.15], ["$-0.15$", r"$\alpha$ in $\mathrm{rad}$", "$0.15$"]
)
Veh_ax_S1_plt.set_ylabel(r"$\mu_\mathrm{f}$")
Veh_ax_S1_plt.plot(
    Veh_alpha_plot, Veh_mu_true_plot, color="red", linestyle=":", label="true"
)
...
handles, labels = Veh_ax_S1_plt.get_legend_handles_labels()
new_handles = [(handles[0], Veh_ax_S1_plt.collections[0]), handles[1]]
labels[0] = r"Alg.\,2 $\pm 3\sigma$"
Veh_ax_S1_plt.legend(
    handles=new_handles,
    labels=labels,
    labelspacing=0.00,
    handlelength=1.0,
...
EMPS_ax_S1_plt.set_xticks(
    [-0.1, 0, 0.1], ["$-0.1$", r"$\dot{q}$ in m/s", "$0.1$"]
)
EMPS_ax_S1_plt.set_ylabel(r"$F$ in N")
...
# the figure
fig_fcn = plt.figure(dpi=150)
gs_fcn = matplotlib.gridspec.GridSpec(
    3, 3, figure=fig_fcn, height_ratios=(8, 10, 10)
)
...
SMO_cax_S1_title_ax = fig_fcn.add_subplot(gs_fcn_01[0, 2])
SMO_cax_S1_title_ax.text(0.5, 0.5, "Error in N", ha="center", va="center")
SMO_cax_S1_title_ax.axis("off")
...
SMO_cax_S2_title_ax = fig_fcn.add_subplot(gs_fcn_02[0, 2])
SMO_cax_S2_title_ax.text(0.5, 0.5, "Error in N", ha="center", va="center")
SMO_cax_S2_title_ax.axis("off")
...
SMO_ax_wRMSE.set_title("Nonlinear Oscillator")
Veh_ax_wRMSE.set_title("Vehicle")
EMPS_ax_RMSE.set_title("Real-world Positioning System")


### make trajectory plot

plot_Data(
    Particles=np.concatenate([SMO_online_Sigma_X, SMO_online_Sigma_F], axis=-1),
...
plot_Data(
    Particles=np.concatenate(
        [Veh_online_Sigma_X, Veh_online_Sigma_mu_f], axis=-1
    ),
...
plot_Data(
    Particles=np.concatenate(
        [EMPS_online_Sigma_X, EMPS_online_Sigma_F], axis=-1
    ),
...
SMO_ax_wRMSE.plot(SMO_time, np.mean(SMO_online_logL, axis=1), color=imes_blue)
SMO_ax_wRMSE.plot(
    SMO_time,
    np.mean(SMO_offline_logL, axis=1),
    color="gray",
    linestyle=":",
)
SMO_ax_wRMSE.set_ylabel(r"$\log p(\boldsymbol{y})$")
SMO_ax_wRMSE.set_xlim(SMO_time[0], SMO_time[-1])
SMO_ax_wRMSE.set_xticks(
    [0, 5, 10, SMO_time[-1]], ["$0$", r"Time in s", "$10$", "$T$"]
)
...
# wRMSE over iterations
Veh_ax_wRMSE.plot(
    Veh_time[3:], np.mean(Veh_online_logL[3:], axis=1), color=imes_blue
)
Veh_ax_wRMSE.plot(
    Veh_time[3:],
    np.mean(Veh_offline_logL[3:], axis=1),
    linestyle=":",
    color="gray",
)
Veh_ax_wRMSE.set_ylabel(r"$\log p(\boldsymbol{y})$")
Veh_ax_wRMSE.set_xlim(Veh_time[0], Veh_time[-1])
Veh_ax_wRMSE.set_ylim(-500, 50)
Veh_ax_wRMSE.set_xticks(
    [0, 10, 20, Veh_time[-1]], ["$0$", r"Time in s", "$20$", "$T$"]
)
...
Veh_ax_S1_plt.plot(
    Veh_alpha_plot, Veh_offline_fcn_mean_f, color="gray", linestyle=":"
)
Veh_ax_S1_plt.set_xticks(
    [-0.15, 0, 0.15], ["$-0.15$", r"$\alpha$ in $\mathrm{rad}$", "$0.15$"]
)
...
Veh_ax_S1_plt.set_xlim(-0.19, 0.19)
Veh_ax_S1_histx.set_ylim(0, 400)
Veh_ax_S1_histx.set_title(f"Time: ${np.round(APF_slice1, 1)}$" + r"$\cdot T$")
# Veh_ax_S1_histx.text(-0.31,227,r'$\# \mathrm{Data}$')
...
Veh_ax_S2_plt.plot(
    Veh_alpha_plot,
    Veh_offline_fcn_mean_f,
    color="gray",
    linestyle=":",
    label=r"Alg.\,2",
)
Veh_ax_S2_plt.set_xticks(
    [-0.15, 0, 0.15], ["$-0.15$", r"$\alpha$ in $\mathrm{rad}$", "$0.15$"]
)
...
Veh_ax_S2_histx.set_title(r"Time: $1.0\cdot T$")
handles, labels = Veh_ax_S2_plt.get_legend_handles_labels()
new_handles = [
    (handles[0], Veh_ax_S2_plt.collections[0]),
    handles[1],
    handles[2],
]
labels[0] = r"Alg.\,1 $\pm 3\sigma$"
Veh_ax_S2_plt.legend(
    handles=new_handles,
    labels=labels,
    labelspacing=0.00,
    handlelength=1.0,
...
EMPS_online_wRMSE = calc_wRMSE(
    1 / fcn_var_online, fcn_mean_online, EMPS_offline_fcn_mean_offline
)

# wRMSE over time
EMPS_ax_RMSE.plot(
    EMPS_time,
    np.mean(EMPS_online_logL, axis=1),
    color=imes_blue,
    label=r"Alg.\,1",
)
EMPS_ax_RMSE.plot(
    EMPS_time,
    np.mean(EMPS_offline_logL, axis=1),
    color="gray",
    linestyle=":",
    label=r"Alg.\,2",
)
EMPS_ax_RMSE.plot(
    EMPS_time,
    np.mean(EMPS_offline_logL_PGAS, axis=1),
    color="g",
    linestyle="--",
    label="PGAS [39]",
)
EMPS_ax_RMSE.set_ylabel(r"$\log p(\boldsymbol{y})$")
EMPS_ax_RMSE.set_xlim(EMPS_time[0], EMPS_time[-1])
EMPS_ax_RMSE.set_xticks(
    [0, 10, 20, EMPS_time[-1]], ["$0$", r"Time in s", "$20$", "$T$"]
)
EMPS_ax_RMSE.legend(
    labelspacing=0.00,
    handlelength=1.0,
...
EMPS_ax_S1_plt.plot(
    EMPS_dq_plot, EMPS_offline_fcn_mean_offline, color="gray", linestyle=":"
)
EMPS_ax_S1_plt.set_xticks(
    [-0.1, 0, 0.1], ["$-0.1$", r"$\dot{q}$ in m/s", "$0.1$"]
)
...
apply_basic_formatting(fig_fcn, width=18, height=12.2, font_size=8)
