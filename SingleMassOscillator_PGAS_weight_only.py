...
def conditional_SMC_with_ancestor_sampling(Y, U,                    # observations and inputs
    ...
    F_particles = F_spring(X_particles[:,0]) + F_damper(X_particles[:,1])
...
def parameter_update(X_prop_i, P_prop_i, F_prop_i,
                    X_prior, P_prior, P_cov_known, P_bounds,
                    f_x, basis_func, U,
                    dt, seed):
    # P_bounds = [0,5]
    ...
    P_out = P_prop_i[None,...]
    P_stats_new = list(prior_n_calcStatistics(*P_prior, P_out))

    # use suff. statistics to calculate posterior NIW parameters
    P_mean, sample_number = prior_n_2naturalPara_inv(*P_stats_new)
    ...
    P_cov_col = np.linalg.cholesky(P_cov/sample_number)
    while True:
...
if __name__ == '__main__':
    ...
    eta_0, eta_1  = prior_n_2naturalPara(P_mean, sample_number) # storing natural parameters
    P_prior = [eta_0, eta_1]
    ...
    X_prop_im1, _, F_prop_i, weight_depletion = conditional_SMC_with_ancestor_sampling(Y, U,                     # observations and inputs
                                                        X_iw_sample, P_n_sample, #F_mniw_sample,   # parameters
                                                        x0, X_prop_im1,                                 # reference trajectory
                                                        f_x, f_y, basis_fcn, R,                             # model structures (including known covariance R)
                                                        200, dt, X, M, F_sd, 0, ancestor_sampling = True)                                      # further settings
    ...
    for i in tqdm(range(1,steps), desc="Running identification"):
        ...
        X_iw_sample, P_n_sample = parameter_update(X_prop_i, P_prop_i, F_prop_i,       # samples for the latent variable sequences
                                                                X_prior, P_prior, P_cov_known, P_bounds,#F_prior,              # initial values for parameter priors
