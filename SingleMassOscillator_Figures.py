...
GP_prior = prior_mniw_2naturalPara_inv(
    GP_prior_stats[0], GP_prior_stats[1], GP_prior_stats[2], GP_prior_stats[3]
)
...
for i in index:
    ...
    fig_fcn_e.savefig(
        f"plots/SingleMassOscillator_PGAS_Fsd_fcn_{int(c)}.pdf",
        bbox_inches="tight",
    )
    c += 1
...
wRMSE = np.sqrt(
    1
    / (v1 - (v2 / v1**2))
    * jnp.sum((fcn_mean - F_sd_true_plot) ** 2 * w, axis=-1)
)
fig_RMSE, ax_RMSE = plt.subplots(1, 1, layout="tight")
ax_RMSE.plot(np.array(range(0, N_iterations)) + 1, wRMSE, color=imes_blue)
...
fig_RMSE.savefig(
    "plots/SingleMassOscillator_PGAS_Fsd_wRMSE.pdf", bbox_inches="tight"
)
...
for i in index:
    ...
    apply_basic_formatting(fig_fcn_e, width=8, height=8, font_size=8)
    fig_fcn_e.savefig(
        f"plots/SingleMassOscillator_APF_Fsd_fcn_{int(c)}.pdf",
        bbox_inches="tight",
    )
    c += 1
...
apply_basic_formatting(fig_RMSE, width=8, height=8, font_size=8)
fig_RMSE.savefig(
    "plots/SingleMassOscillator_APF_Fsd_wRMSE.pdf", bbox_inches="tight"
)
