...
from src.BayesianInferrence import (
    prior_mniw_Predictive,
    prior_mniw_2naturalPara_inv,
)
...
(
    offline_GP_Mean_f,
    offline_GP_Col_Cov_f,
    offline_GP_Row_Scale_f,
    offline_GP_df_f,
) = jax.vmap(prior_mniw_2naturalPara_inv)(
    GP_prior_stats_f[0]
    + np.cumsum(offline_T0_f, axis=0)
    / np.arange(1, offline_Sigma_X.shape[1] + 1)[:, None, None],
    GP_prior_stats_f[1]
    + np.cumsum(offline_T1_f, axis=0)
    / np.arange(1, offline_Sigma_X.shape[1] + 1)[:, None, None],
    GP_prior_stats_f[2]
    + np.cumsum(offline_T2_f, axis=0)
    / np.arange(1, offline_Sigma_X.shape[1] + 1)[:, None, None],
    GP_prior_stats_f[3]
    + np.cumsum(offline_T3_f, axis=0)
    / np.arange(1, offline_Sigma_X.shape[1] + 1),
)
(
    offline_GP_Mean_r,
    offline_GP_Col_Cov_r,
    offline_GP_Row_Scale_r,
    offline_GP_df_r,
) = jax.vmap(prior_mniw_2naturalPara_inv)(
    GP_prior_stats_r[0]
    + np.cumsum(offline_T0_r, axis=0)
    / np.arange(1, offline_Sigma_X.shape[1] + 1)[:, None, None],
    GP_prior_stats_r[1]
    + np.cumsum(offline_T1_r, axis=0)
    / np.arange(1, offline_Sigma_X.shape[1] + 1)[:, None, None],
    GP_prior_stats_r[2]
    + np.cumsum(offline_T2_r, axis=0)
    / np.arange(1, offline_Sigma_X.shape[1] + 1)[:, None, None],
    GP_prior_stats_r[3]
    + np.cumsum(offline_T3_r, axis=0)
    / np.arange(1, offline_Sigma_X.shape[1] + 1),
)
...
for i in index:
    ...
    fig_fcn_e.savefig(
        f"plots/Vehicle_PGAS_muf_fcn_{int(c)}.pdf", bbox_inches="tight"
    )
    c += 1
...
wRMSE_f = np.sqrt(
    1
    / (v1 - (v2 / v1**2))
    * jnp.sum((fcn_mean_f - mu_true_plot) ** 2 * w, axis=-1)
)
...
wRMSE_r = np.sqrt(
    1
    / (v1 - (v2 / v1**2))
    * jnp.sum((fcn_mean_r - mu_true_plot) ** 2 * w, axis=-1)
)

fig_RMSE, ax_RMSE = plt.subplots(1, 1, layout="tight")
ax_RMSE.plot(range(0, N_PGAS_iter), wRMSE_f, color=imes_blue)
ax_RMSE.plot(range(0, N_PGAS_iter), wRMSE_r, color=imes_orange)
...
for i in index:
    ...
    apply_basic_formatting(fig_fcn_e, width=8, height=8, font_size=8)
    fig_fcn_e.savefig(
        f"plots/Vehicle_APF_muf_fcn_{int(c)}.pdf", bbox_inches="tight"
    )
    c += 1
...
fig_RMSE, ax_RMSE = plt.subplots(1, 1, layout="tight")
ax_RMSE.plot(time, wRMSE_f, color=imes_blue)
ax_RMSE.plot(time, wRMSE_r, color=imes_orange)
ax_RMSE.set_ylabel(r"wRMSE")
